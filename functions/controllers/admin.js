      ...
      db.child(users)
        ...
        .then((snapshot) => {
          ...
          for (user in allUsers) {
            ...
            if (
              allUsers[user][registeredEvents][eventCategory].indexOf(
                eventName
              ) !== -1
            ) {
...
async function emailtoarray(req, res) {
  ...
  if (
    ...
    return res.status(400).json({
      ...
      error:
        "Usage: users, heading, buttontext, buttonlink, thankyou, subject, detail are required",
    });
  }
